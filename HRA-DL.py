...
def main(userData):	
	...
	print(f"{albumFolder}\n")
	albumFolderS = dirSetup(f"/content/drive/MyDrive/Hi-Res/HRA/{sanitizeFname(albumFolder)}")
	for tracks in [x for x in metadata['data']['results']['tracks']]:
		preFname = f"/content/drive/MyDrive/Hi-Res/HRA/{sanitizeFname(albumFolder)}/{str(tracks['trackNumber']).zfill(2)}.flac"
		postFname = f"/content/drive/MyDrive/Hi-Res/HRA/{sanitizeFname(albumFolder)}/{str(tracks['trackNumber']).zfill(2)}. {sanitizeFname(tracks['title'])}.flac"
		fileSetup(preFname)
		fileSetup(postFname)
